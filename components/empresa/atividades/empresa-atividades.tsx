...
function getDisplaySeconds(session: ActiveSession): number {
  if (session.status === 'running' && session.resumedAt) {
    return session.totalSeconds + Math.floor((Date.now() - new Date(session.resumedAt).getTime()) / 1000);
  }
...
function groupByMember(sessions: ActiveSession[]): MemberGroup[] {
  ...
  for (const s of sessions) {
    if (!map.has(s.userId)) {
      map.set(s.userId, { userId: s.userId, userName: s.userName, userPhotoUrl: s.userPhotoUrl, sessions: [] });
    }
...
interface EmpresaAtividadesProps {
  initialSessions: RawSession[];
  companyId: string;
  token: string;
}

export function EmpresaAtividades({ initialSessions, companyId, token }: EmpresaAtividadesProps) {
  const [sessions, setSessions] = useState<ActiveSession[]>(() => initialSessions.map(rawToActiveSession));
  ...
  useEffect(() => {
    ...
    socket.on('reconnect', () => socket.emit('joinCompany', { companyId }));
    ...
    socket.on('taskSession:paused', (p: { sessionId: string; totalSeconds: number }) => {
      setSessions((prev) =>
        prev.map((s) => s.id === p.sessionId ? { ...s, status: 'paused', totalSeconds: p.totalSeconds, resumedAt: null } : s),
      );
    });

    socket.on('taskSession:resumed', (p: { sessionId: string; resumedAt: string }) => {
      setSessions((prev) =>
        prev.map((s) => s.id === p.sessionId ? { ...s, status: 'running', resumedAt: p.resumedAt } : s),
      );
    });
  ...
  if (memberGroups.length === 0) {
    return (
      <div className="rounded-lg border bg-card px-6 py-16 text-center space-y-2">
        ...
        <p className="text-xs text-muted-foreground">As tarefas iniciadas pelos membros aparecerão aqui em tempo real.</p>
      </div>
    );
  }
              ...
              <div className="divide-y">
                {group.sessions.map((session) => (
                  <div key={session.id} className="flex items-center gap-3 px-4 py-3">
                    <span className={`h-2 w-2 shrink-0 rounded-full ${session.status === 'running' ? 'bg-green-500 animate-pulse' : 'bg-yellow-500'}`} />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium truncate" title={session.taskTitle}>{session.taskTitle}</p>
                      <p className="text-xs text-muted-foreground">
                        {session.projectName}
                        {session.workspaceName && (
                          <span className="text-muted-foreground/60"> · {session.workspaceName}</span>
                        )}
                      </p>
                    </div>
                    ...
                    <Link
                      href={`/workspace/${session.workspaceId}/projetos/${session.projectId}?task=${session.taskId}`}
